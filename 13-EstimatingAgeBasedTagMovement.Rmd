...
\]
This selectivity is forced to have maximum value of one so that all fish in the partition move based on either \(\boldsymbol{M}^y\) and \(\boldsymbol{M}^o\)
\[
S^o_a = \frac{S^o_a}{max(\boldsymbol{S}^o)}
\]
...
The following section is code and plots from running the three follosing simulations,
