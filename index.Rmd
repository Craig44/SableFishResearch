...
- Chapter \@ref(modeldescription) documents the current stock assessment model and assumptions (work in progress). This is the first step of the project, with the purpose of helping me understand the assessment, data and important dynamics.

- Chapter \@ref(spatialmodeldescription) documents a generalised spatial stock assessment model used as both an OM and EM during simulations.


- Chapter \@ref(surveydata) describes, characterises and explores the survey longline data.


- Chapter \@ref(observerdata) describes, characterises and explores fishery dependent data, which includes reported catch and observer records.
...
- When is the latest you can start the model? Currently the assessment model starts in 1960 because there is early survey and catch info along with some of the largest catches observed over the fishery. I want to explore starting the model at later period i.e., the 1980's when there is consistent surveys and age data. This is thought to reduce the number of estimable parameters that have low information (early recruitment). However, the downfall is we may loose information on stock productivity because the 60's and 70's have some of the largest recorded catches.
