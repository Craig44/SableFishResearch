...
### Fishing mortality {-}
When the hybrid F method was employed by the model, tagged fish were not included when internally solving the fishing mortality nuisance parameters. The ratio of tagged to untagged numbers of fish in the partition at any point was thought to be small enough not to effect F estimates. We wanted to reduce the computational overhead this would require.
...
Most of the tag releases have unknown sex. Sex disaggregated age-frequencies were derived using an ALK that was disaggregrated by sex in the age dimension but sex aggregated in the length dimension. This meant when we multiplied an unsexed length frequency through this ALK we get sex disaggregated age frequency. 

Age length keys were not available in all years when tagged fish were released, particularly in the early period. Tag release age frequencies before 1981 were based on age-length keys from 1981, 1985 and 1987. After that period if an age-length key was not available we used the age length key from the closest year with an age-length key.
...
All tag recoveries are from the fixed gear fishery, due to the recent trend in recoveries by gear method (Figure \@ref(fig:recoveriebygear)), we only considered tags recovered before 2018. With the decline in the longline catch, recoveries have declined substantially. The switch to pot gear has not seen a recovery rate that is close to the longline method. To avoid this complication we will only recoveries from a period when the longline method was the predominant method of fixed gear methods. 

Model fitted values for tags recovered in region \(r\) and year \(y\) that were released by tag-release event \(k\) denoted as \(\widehat{N}^k_{a,r,y,s}\) are derived as


\[
\widehat{N}^k_{a,r,y,s} =  N^k_{a,r,y,s} 1 - e^{-F^g_{a,r,y,s} + 0.5M_{y}} \delta_y
\]
where, \(N^k_{a,r,y,s}\) are the number of tagged fish from tag-release event \(k\), and \(\delta_y\) is the reporting rate in year \(y\).
