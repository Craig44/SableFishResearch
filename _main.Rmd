...
- [x]  ~~Conduct self test using TMB stock assessment model. Planned date of completion is December 1 2022~~ I decided not to do this because the assessment had too many bespoke likelihoods
...
- [ ]  Calculate mean length at age for each year - perhaps use the growth model, but this may be double using some of the datasets? This is an interesting idea cause if we take the tag data for example it would be used to estimate mean length at age just using increment and time at liberty, where as when it is internally used in the model it would be using age-frequency to estimate ontogenetic movement.
...
where, \(\boldsymbol{\theta}\) denotes "fixed-effect" parameters which are furthur seperated by \(\boldsymbol{\theta} = (\boldsymbol{\theta^f},\boldsymbol{\theta^h}) \), with \(\boldsymbol{\theta^h}\) denoting fixed effects that are hyperprior parameters for the "random-effect" variables denoted by \(\boldsymbol{u}\) and \(\(\boldsymbol{\theta^h}\) \) are all other fixed-effect parameters.
...
The latest published stock assessment [@goethel2021assessment] assumes a panmictic stock. However, there are many studies highlighting spatial heterogeneity in sablefish in the Alaskan region [@heifetz1991movement;@hanselman2015move]. This chapter descripes an extension of the current assessment model by including a spatial dimension and tagged fish in the partition. Let \(\boldsymbol{N_{r,y,s}}\) denote a vector of ages in year \(y\) sex \(s\) in region \(r\) (the partition) i.e., \(\boldsymbol{N_{r,y,s}} = (N_{1,r,y,s}, N_{2,r,y,s}, \dots, N_{a_+,r,y,s})^T\). 
...
Do we want to include the inshore (Chatham Strait and Clarence Strait) tag-releases? there are alot of fish tagged in these regions. Also do we care about the amount of tags that captured off Canada and US? See Figure \@ref(fig:TagRecoveriesOutsideRegion)
...
All tag recoveries are from the fixed gear (longline and pot gear) fishery, due to the recent trend in recoveries by gear method (Figure \@ref(fig:recoveriebygear)), we only considered tags recovered before 2018. The switch from longline to pot fishing in the fixed gear fisheyr has not seen anywhere the same recoveries to be reported from the pot gear compared to the longling gear. To avoid this complication we will only recoveries from a period when the longline method was the dominant method of fixed gear catch 
...
- different densities i.e. negative binomial
...
- How best to pool data with low sample sizes in a year? how to set these sample size thresholds?
- How to deal with ages that have no otoliths? set zero or impute?


### Age length keys {-}
Calculate the probability of age given a length bin by using the empirical distribution of age and lengths.

\[
\hat{p}(a|l) = \frac{n_{a,l}}{n_{+,l}}
\]
where, \(n_{+,l}\) is the total of all ages in length bin \(l\).
The probabilities of age given length from the forward ALK are then simply multiplied by the marginal probabilities \(\hat{p}(l) = N_{l} / \sum_lN_{l}\).
...
This method was not explored in detail due to the sex dimension for sablefish adding a complexity we didn't have time to get in to. There is R code that applies the CRL model ignoring sex in the data.
...
\begin{align*}
T^k_{r,t + 1} & = T^k_{r,t} e^{-Z_t} \\
Z_t & = M + F_t \\
\boldsymbol{T}^k_{t + 1} & = \boldsymbol{T}^k_{t + 1} \boldsymbol{M}
\end{align*}
where, \(\boldsymbol{T}^k_{t + 1} =  (T^k_{1,t + 1},\dots, T^k_{n_r,t + 1})\) is a vector of numbers of tagged fish across all regions and \(\boldsymbol{M}\) is a \(n_r \times n_r\) movement matrix. This assumes each region has the same fishing mortality in each time-step.
...
\]
where, \(N^k_{NR}\) represents the not recovered category and is \(N^k_{NR} = T^k - \sum\limits_m^{n_m}\), where \(T^k\) is the initial number of tags released for event \(k\). \(\widehat{\boldsymbol{\theta}}^k\) is a vector of proportions that sum to one, with the same dimensions as \(\boldsymbol{N}^k\) and is calculated as,
...
The following is some R-code that was used to generate operatint model values for 
```{r simulate_tag_data}
set.seed(123)
n_y = 5
n_regions = 3
...
for(over_ndx in 1:length(over_dispersion_params)) {
  ...
  for(sim in 1:n_sim) {
    ...
    recap_mat$sim = sim
    multi_recap_mat = melt(multi_recap_rep$movement_matrix)
    multi_recap_mat$RE = (multi_recap_mat$value - OM_mat$proportion) / OM_mat$proportion * 100
    multi_recap_mat$SE = (OM_mat$proportion - multi_recap_mat$value)^2
    multi_recap_mat$type = "Multinomial Recapture"
    multi_recap_mat$sim = sim
    ...
    nb_mat$sim = sim
    full_move_df = rbind(full_move_df, rbind(pois_mat, recap_mat, multi_recap_mat, multi_release_mat, nb_mat))
  }
...
kable(sim_data_info, caption = "Average proportion of zeros and variance of simulated data for different overdispersion settings.")
```


```{r taglikesummaryplotsRE, echo = F, eval = T, warning=FALSE, message=FALSE, error = FALSE, fig.cap="Relative error in movement parameters. Columns are overdispersion values for simulated data. GGplot facets are movement matrix elemen combinatons"}
...
```{r taglikesummaryplotsEst, echo = F, eval = T, warning=FALSE, message=FALSE, error = FALSE, fig.cap="Estimated movement parameters, red line is true value. Columns are overdispersion values for simulated data. GGplot facets are movement matrix elemen combinatons"}
...
<!--chapter:end:07-TagLikelihoods.Rmd-->
...
<!--chapter:end:10-ReferencePoints.Rmd-->
